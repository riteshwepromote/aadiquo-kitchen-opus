...
function KitchenIsland({ scrollProgress }: { scrollProgress: React.MutableRefObject<number> }) {
  ...
  useFrame((state) => {
    ...
    if (drawer1.current)
      drawer1.current.position.z = THREE.MathUtils.lerp(
        drawer1.current.position.z,
        0.05 + p * 0.7,
        0.08,
      );
    if (drawer2.current)
      drawer2.current.position.z = THREE.MathUtils.lerp(
        drawer2.current.position.z,
        0.05 + p * 0.5,
        0.08,
      );
    if (cabinetL.current)
      cabinetL.current.rotation.y = THREE.MathUtils.lerp(
        cabinetL.current.rotation.y,
        -p * 1.1,
        0.08,
      );
    if (cabinetR.current)
      cabinetR.current.rotation.y = THREE.MathUtils.lerp(
        cabinetR.current.rotation.y,
        p * 1.1,
        0.08,
      );
  ...
  return (
    <group ref={group} position={[0, -0.2, 0]}>
      ...
      <mesh position={[-1.81, 0.5, 0]} material={matte}>
        <boxGeometry args={[0.02, 1, 1.4]} />
      </mesh>
      <mesh position={[1.81, 0.5, 0]} material={matte}>
        <boxGeometry args={[0.02, 1, 1.4]} />
      </mesh>
      ...
      {/* brass handles */}
      <mesh position={[-1, 0.7, 0.08]} material={brass}>
      ...
      <group ref={cabinetL} position={[-2.8, 1.3, -0.6]}>
        ...
        <mesh position={[0.81, 0, 0]} material={brass}>
